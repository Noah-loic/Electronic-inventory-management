...
export default function PortalLayout() {
    ...
    const visibleLinks = PORTAL_NAV_ITEMS.filter(item => !item.permission || auth?.permissions?.includes(item.permission))
        ...
        <div className="flex h-screen bg-gray-100 overflow-hidden">
            <aside className={`${sidebarOpen ? 'w-60' : 'w-16'} bg-gray-900 text-white flex flex-col transition-all duration-200 shrink-0`}>
                <div className="flex items-center gap-3 px-4 py-5 border-b border-gray-700">
                    <div className="w-8 h-8 bg-indigo-500 rounded-lg shrink-0 flex items-center justify-center font-bold text-sm">E</div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shrink-0">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full font-medium">{auth?.roles?.join(', ')}</span>
